...
export default function UserManagementPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#030712] text-slate-200 font-sans selection:bg-indigo-500/30 relative overflow-hidden">
                      ...
                      >
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                            ...
                            {!user.isDeleted && (
                              <button onClick={() => handleEditClick(user)} className="p-2 rounded-lg hover:bg-emerald-500/10 hover:text-emerald-400 text-slate-400 transition-all">
                            ...
                            <div className="relative">
                              <button onClick={() => setActionMenuOpen(actionMenuOpen === user._id ? null : user._id)} className={`p-2 rounded-lg hover:bg-slate-700/50 text-slate-400 transition-all ${actionMenuOpen === user._id ? 'bg-slate-700 text-white' : ''}`}>
                              ...
                              {actionMenuOpen === user._id && (
                                <>
                                  ...
                                  <div className="absolute right-0 top-full mt-2 w-48 rounded-xl bg-[#0F172A] border border-white/10 shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                    {!user.isDeleted ? (
                                      <>
                                        <button onClick={() => handleToggleActive(user._id)} className="w-full text-left px-4 py-3 text-xs font-medium text-slate-300 hover:bg-white/5 hover:text-white transition-colors flex items-center gap-2">
                                          <Ban className="h-3.5 w-3.5" />
                                          {user.isActive ? "Block Access" : "Unblock User"}
                                        </button>
                                        <button onClick={() => { setSelectedUser(user); setArchiveModalOpen(true); }} className="w-full text-left px-4 py-3 text-xs font-medium text-amber-400 hover:bg-amber-500/10 transition-colors flex items-center gap-2 border-t border-white/5">
                                          <Archive className="h-3.5 w-3.5" />
                                          Archive User
                                        </button>
                                      </>
                                    ) : (
                                      <button onClick={() => { setSelectedUser(user); setRestoreModalOpen(true); }} className="w-full text-left px-4 py-3 text-xs font-medium text-emerald-400 hover:bg-emerald-500/10 transition-colors flex items-center gap-2">
      ...
      {/* --- MODALS (Code remains mostly the same, just logic triggers) --- */}
      {/* View User Modal */}
