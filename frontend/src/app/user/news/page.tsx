...
import { Search, RefreshCw, Filter, Moon, Sun, TrendingUp, Zap, Globe, Newspaper, ArrowUpRight, Hash } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-3">
             ...
             <button
              ...
              className={`p-2.5 rounded-xl border transition-all ${isDark ? 'bg-slate-900 border-slate-800 text-yellow-400 hover:bg-slate-800' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
            >
                ...
                <div className={`rounded-2xl p-5 border backdrop-blur-md ${theme.sidebarBg}`}>
                    ...
                    <div className="space-y-3">
                        {trendingSymbols.length > 0 ? trendingSymbols.map(([symbol, count], i) => (
                            <div key={symbol} className={`group flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer ${isDark ? 'bg-slate-950/50 border-slate-800 hover:border-indigo-500/30 hover:bg-slate-900' : 'bg-slate-50 border-slate-100 hover:border-indigo-200 hover:bg-white'}`}>
                                <div className="flex items-center gap-3">
