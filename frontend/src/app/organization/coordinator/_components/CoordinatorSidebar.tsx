...
const SidebarHeader = memo(({ isOpen, onToggle, deptName }: { isOpen: boolean; onToggle: () => void; deptName: string }) => {
    return (
        <div className="h-[88px] flex items-center px-6 border-b border-white/5 relative shrink-0">
            ...
            >
                <div className="relative">
                    <div className="absolute -inset-3 bg-purple-500/20 rounded-full blur-xl opacity-0 group-hover/header:opacity-100 transition-opacity duration-700" />
                    <div className="relative h-11 w-11 rounded-2xl bg-gradient-to-br from-purple-600 to-pink-700 flex items-center justify-center shadow-[0_0_20px_rgba(147,51,234,0.3)] ring-1 ring-white/10 group-hover/header:scale-105 transition-transform duration-300">
                ...
                <div className={cn("flex flex-col overflow-hidden transition-all duration-500", isOpen ? "w-auto opacity-100" : "w-0 opacity-0")}>
                    ...
                    <span className="text-[10px] font-bold text-purple-400 tracking-[0.1em] mt-1 whitespace-nowrap uppercase truncate max-w-[140px]" title={deptName}>
                        {deptName}
                    </span>
                </div>
            </div>
            ...
            {isOpen && (
                <button
                    ...
                    className="absolute -right-3 top-1/2 -translate-y-1/2 h-7 w-7 bg-[#0B0C15] border border-white/10 rounded-full flex items-center justify-center text-slate-400 shadow-xl hover:bg-purple-600 hover:border-purple-500 hover:text-white transition-all duration-300 z-50 group/toggle"
                >
...
const NavItem = memo(({
    ...
    // Purple/Pink theme for Coordinator
    const activeClass = cn(
        "bg-gradient-to-r from-purple-600/20 via-pink-500/10 to-transparent",
        "text-white",
        ...
        <div className="relative group/nav">
            ...
            >
                {(isActive || isSubActive) && (
                    <div className="absolute left-0 w-full h-full bg-purple-500/5 opacity-50 blur-xl pointer-events-none" />
                )}

                <div className={cn(
                    "relative z-10 transition-all duration-300",
                    (isActive || isSubActive) ? "text-purple-400 drop-shadow-[0_0_8px_rgba(192,132,252,0.5)] scale-105" : "text-slate-500 group-hover:text-slate-300"
                )}>
...
const StatusCard = memo(() => {
    return (
        <div className="relative rounded-2xl bg-gradient-to-br from-purple-900/50 to-pink-900/50 p-5 shadow-lg overflow-hidden group border border-white/10 hover:border-white/20 transition-colors">
        ...
        <div className="flex-shrink-0 p-4 border-t border-white/5 bg-[#08090f] relative group">
            <div className={cn("flex items-center gap-3 p-2 rounded-2xl transition-all duration-300", isOpen ? "bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10" : "justify-center")}>
                    ...
                    <div className="h-full w-full rounded-full p-[2px] bg-gradient-to-tr from-purple-500 to-pink-500 shadow-lg relative z-10">
                        <div className="h-full w-full rounded-full bg-[#0B0C15] overflow-hidden flex items-center justify-center">
                            {!imgError ? (
                                ...
                                <span className="font-bold text-white text-sm bg-gradient-to-br from-purple-500 to-pink-600 w-full h-full flex items-center justify-center">
                ...
                <div className={cn("flex-1 min-w-0 transition-all duration-300", isOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 hidden")}>
                    <p className="text-sm font-bold text-white truncate leading-tight group-hover:text-purple-300 transition-colors">
                        {user.name}
                    </p>
                    <p className="text-[10px] text-slate-500 truncate font-medium uppercase tracking-wide mt-0.5">Coordinator</p>
                </div>
...
export default function CoordinatorSidebar() {
    ...
    const [coordData, setCoordData] = useState({
        ...
        email: "Loading...",
        dept: "Department",
        avatar: "/avatars/coord-default.png"
    });
    ...
    const fetchData = async () => {
        try {
            ...
            if (cData.success && cData.coordinator) {
                setCoordData({
                    name: cData.coordinator.name || "Coordinator",
                    email: cData.coordinator.email || "coordinator@org.com",
                    dept: cData.coordinator.department?.departmentName || "Department",
                    avatar: "/avatars/coord-default.png"
                });
            }
    ...
    const MENU_GROUPS: MenuGroup[] = [
        ...
        },
        {
            ...
            items: [
                {
                    href: "/organization/coordinator/students/pending",
        ...
        },
        {
            title: "System",
            items: [
                { href: "/organization/coordinator/profile", label: "My Profile", icon: Users },
            ],
        },
    ];
    ...
    return (
      ...
      `}</style>
            ...
            >
                ...
                <SidebarHeader isOpen={isOpen} onToggle={handleToggle} deptName={coordData.dept} />
                ...
                <UserProfileFooter isOpen={isOpen} user={coordData} />
            </aside>
        </>
    );
}
