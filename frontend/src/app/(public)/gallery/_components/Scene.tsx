...
const Particles = () => {
    ...
    useFrame((state, delta) => {
        if (ref.current) {
            // Rotate the entire particle system based on scroll
            ref.current.rotation.y -= delta * 0.05 * (1 + scroll.range(0, 1) * 2);
            ref.current.rotation.x = scroll.offset * 0.2;
        }
    });
...
const AbstractShape = () => {
    ...
    useFrame((state, delta) => {
        ...
        const r1 = scroll.range(0, 1 / 3); // Hero section

        // Rotate constantly + extra when scrolling
        meshRef.current.rotation.x += delta * 0.2;
        meshRef.current.rotation.y += delta * 0.3;

        // Move based on scroll
        meshRef.current.position.y = THREE.MathUtils.lerp(0, -viewport.height * 2, scroll.offset);
    });
