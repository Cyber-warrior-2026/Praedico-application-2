...
export default function Navbar() {
  ...
  const navItems = ["Home", "Product", "Gallery", "Markets", "News", "Contacts", "Docs"];
  ...
  return (
    <>
      ...
      >
        ...
        {/* MOBILE MENU */}
        {mobileMenuOpen && (
          <div className="absolute top-full left-0 w-full h-[calc(100vh-80px)] bg-[#020617] border-b border-slate-800 p-6 md:hidden flex flex-col gap-4 animate-in slide-in-from-top-5 overflow-y-auto z-[99]">
            {navItems.map((item) => (
              <Link
                key={item}
                href={item === "Home" ? "/" : `/${item.toLowerCase()}`}
                onClick={() => setMobileMenuOpen(false)}
                className="block px-6 py-4 text-lg font-medium text-slate-200 hover:text-white hover:bg-white/5 transition-all rounded-xl"
              >
                {item}
              </Link>
            ))}
            <div className="h-px bg-slate-800 my-2" />
            <button onClick={() => { setMobileMenuOpen(false); handleSignIn(); }} className="w-full py-4 rounded-xl bg-slate-800 text-white font-medium active:bg-slate-700">
              Sign In
            </button>
            <button onClick={() => { setMobileMenuOpen(false); handleGetStarted(); }} className="w-full py-4 rounded-xl bg-indigo-600 text-white font-medium active:bg-indigo-700">
              Get Started
            </button>
          </div>
        )}
