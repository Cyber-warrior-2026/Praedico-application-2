...
export class UserController {
  ...
  getMe = asyncHandler(async (req: Request, res: Response) => {
    ...
    const displayName = user.name && user.name !== "User" ? user.name : user.email.split("@")[0];
  ...
  logout = asyncHandler(async (req: Request, res: Response) => {
    // 🟢 UPDATED: Matches the new cookie settings
    const cookieOptions = {
        httpOnly: true,
        secure: true, // Always secure in production
        sameSite: "lax" as const, // Match the setAuthCookies setting
        path: "/"
    };
  ...
  // 🟢 CRITICAL FIX FOR PROXY SETUP
  private setAuthCookies(
    res: Response,
    accessToken: string,
    refreshToken: string,
  ) {
    // With the Next.js Rewrite (Proxy), the browser sees this as a Same-Site request.
    // So we use 'lax' instead of 'none'.
    const cookieOptions = {
      httpOnly: true,
      secure: true, // Required for Vercel
      sameSite: "lax" as const, // 'Lax' is best for proxy/same-site
  ...
  // --- Standard CRUD Methods (No changes needed below) ---
